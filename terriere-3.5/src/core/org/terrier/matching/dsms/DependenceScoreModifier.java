...
public abstract class DependenceScoreModifier  implements DocumentScoreModifier {
	...
	/** weight of SD */
	protected double w_SD = Double.parseDouble(ApplicationSetup.getProperty(
				"proximity.w_SD", "1.0d"));
				
	/** weight of FD */
	protected double w_FD = Double.parseDouble(ApplicationSetup.getProperty(
				"proximity.w_FD", "1.0d"));
	...
	public boolean modifyScores(Index index, MatchingQueryTerms terms, ResultSet set) {
		try {
			...
			} else if (dependency.equals("FD_SD")) {
				doDependency2(index, es, ips, set, phraseTermWeights, true);			
			} else {
	...
	protected void doDependency2(Index index, final EntryStatistics es[], final IterablePosting ips[], ResultSet rs, final double[] phraseTermWeights, boolean SD) throws IOException 
	{	
		System.out.println("FD_SD with W_SD=" + w_SD + " and w_FD=" + w_FD);		
		...
		DOC: for (int k = 0; k < docidsLength; k++) {
			...
			TERM: for (i = 0; i < numPhraseTerms - 1; i++) {
				...
				double s = scoreFDSD(SD, i, ips[i], i+1, ips[i + 1],
						avgDocLen);
				scores[k] += w_SD * (combinedPhraseQTWWeight * w_o * s);
			}
			...
			for (i = 0; i < numPhraseTerms - 1; i++) {
				INNERTERM: for (int j = i + 1; j < numPhraseTerms; j++) {
					...
					scores[k] += w_FD * (w_u * combinedPhraseQTWWeight * s);
				}
			}
